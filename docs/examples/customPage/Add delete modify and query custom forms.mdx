...
接口配置如图：
![image.png](https://img.alicdn.com/imgextra/i2/O1CN01hE5r281Gvm2wvKVPD_!!6000000000685-2-tps-2754-1246.png)
...
1. 在数据源处添加远程数据源，调用新增表单实例接口
接口配置如图：
![image.png](https://img.alicdn.com/imgextra/i3/O1CN011bDI4R1S1EXeadxCK_!!6000000002186-2-tps-2878-1320.png)
2. 添加弹窗组件，配置新增内容的组件到弹窗中
配置如图：
![image.png](https://img.alicdn.com/imgextra/i3/O1CN01VwHloU1mqnRoRZyIi_!!6000000005006-2-tps-2878-1302.png)
添加对话框组件
3. 在「顶部操作 >> 操作条」添加新增的操作按钮，给按钮添加弹窗事件，其中使用 this.$(fieldId).show() 来展示对话框，括号中填入的为对话框的唯一标识符
```javascript
...
1. 配置操作列中的详情操作项
如图所示：
![image.png](https://img.alicdn.com/imgextra/i3/O1CN011dwyq91xYtjxVXcdM_!!6000000006456-2-tps-2768-1192.png)
...
<video width="100%" controls>
 ...
 ### 步骤四：删除数据 
 参考文档：[宜搭平台接口（页面数据源可直接调用）](https://www.yuque.com/yida/support/aql605?view=doc_embed&inner=9026sb)
1. 数据源处加远程数据源，调用删除表单实例接口
接口配置如图：
![image.png](https://img.alicdn.com/imgextra/i4/O1CN01MS8mzk22jxNw95kdf_!!6000000007157-2-tps-2878-1318.png)
2. 添加删除操作列
![image.png](https://img.alicdn.com/imgextra/i1/O1CN01XpXn3C1QYY7ZJumEU_!!6000000001988-2-tps-2780-1260.png)
...
### 步骤五：编辑修改数据
参考文档：[宜搭平台接口（页面数据源可直接调用）](https://www.yuque.com/yida/support/aql605?view=doc_embed&inner=9026sb)
1. 在数据源处添加远程数据源，调用修改表单组件值的接口
接口配置如图：
![image.png](https://img.alicdn.com/imgextra/i1/O1CN01gvwJUe1hwgSSSu3P4_!!6000000004342-2-tps-2878-1312.png)
2. 新增编辑操作列以及保存操作列，由于编辑后会进入编辑态，因此需要将保存按钮放置在编辑态中
配置如图：
![image.png](https://img.alicdn.com/imgextra/i3/O1CN018lhHQr1MDFEOMOjxO_!!6000000001400-2-tps-2772-1296.png)
...
```
![image.png](https://img.alicdn.com/imgextra/i4/O1CN01EKWt0T1JKclWOFQWW_!!6000000001010-2-tps-2870-1272.png)
4. 添加保存事件，在保存事件中加载 Update 数据源，调用接口，并配置延时在修改后重新调用查询表单实例接口，将新数据加载展示，参考代码如下：
...
![image.png](https://img.alicdn.com/imgextra/i1/O1CN01RZdzU61xTrNtHf7L8_!!6000000006445-2-tps-1608-1068.png)
...
![image.png](https://img.alicdn.com/imgextra/i2/O1CN01yBKfBf1hYrrdtFoOc_!!6000000004290-2-tps-1562-816.png)
