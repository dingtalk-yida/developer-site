...
![](https://img.alicdn.com/imgextra/i2/O1CN01j9dlf31hkmA4JWX2h_!!6000000004316-2-tps-1364-462.png_.webp)
基础数据表
**「自定义页面求和」：**自定义页面，使用表格组件，并放入数值组件用来展示求和数据。
![](https://img.alicdn.com/imgextra/i1/O1CN01BlRTTv1mDrmTA7fHV_!!6000000004921-2-tps-2280-718.png_.webp)
自定义页面求和
...
1. 增加远程数据源，调用「根据条件搜索表单实例详情列表」接口
   ...
   ![](https://img.alicdn.com/imgextra/i1/O1CN01L9e3gw1FquDdx0tJO_!!6000000000539-2-tps-2060-1084.png_.webp)
   创建远程数据源，配置获取表单数据接口
2. 将返回数据处理成表格格式
   ![](https://img.alicdn.com/imgextra/i3/O1CN01Ni9AtZ23DGjORMBie_!!6000000007221-2-tps-2080-1044.png_.webp)
   添加请求完成时回调函数（didFench）
   :::info
   在 value 中定义需要使用的内容，同时需要给表格的数据主键配置唯一值，因此将「formInstId」，即实例 ID 作为主键。
   :::
   参考代码如下：
...
在线体验请移步示例中心 👉 [示例中心|自定义页面选择行求和](https://www.aliwork.com/o/coc?tplUuid=TPL_GMKGSQJ4LT0RQBT0MINK&from=developers_subject)
